...
class GetShiftApiV1ShiftsShiftIdGet extends \RevoTale\CheckboxUA\Runtime\Client\BaseEndpoint implements \RevoTale\CheckboxUA\Runtime\Client\Endpoint
{
     ...
     *             }
     */
    public function __construct(string $shiftId, array $headerParameters = [])
    {
    ...
    {
        return str_replace(['{shift_id}'], [$this->shift_id], '/api/v1/shifts/{shift_id}');
    }
    ...
    protected function transformResponseBody(\Psr\Http\Message\ResponseInterface $response, \Symfony\Component\Serializer\SerializerInterface $serializer, string $contentType = null)
    {
        ...
        if (false === is_null($contentType) && (200 === $status && false !== mb_strpos($contentType, 'application/json'))) {
            return $serializer->deserialize($body, 'RevoTale\\CheckboxUA\\Model\\ShiftWithCashierAndCashRegister', 'json');
        }
        if (false === is_null($contentType) && (422 === $status && false !== mb_strpos($contentType, 'application/json'))) {
            throw new \RevoTale\CheckboxUA\Exception\GetShiftApiV1ShiftsShiftIdGetUnprocessableEntityException($serializer->deserialize($body, 'RevoTale\\CheckboxUA\\Model\\HTTPValidationError', 'json'), $response);
        }
