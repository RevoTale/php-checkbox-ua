...
class ExportGoodsApiV1GoodsExportExportExtensionGet extends \RevoTale\CheckboxUA\Runtime\Client\BaseEndpoint implements \RevoTale\CheckboxUA\Runtime\Client\Endpoint
{
     ...
     *             }
     */
    public function __construct(string $exportExtension, array $headerParameters = [])
    {
        $this->export_extension = $exportExtension;
    ...
    {
        return str_replace(['{export_extension}'], [$this->export_extension], '/api/v1/goods/export/{export_extension}');
    }
    ...
    protected function transformResponseBody(\Psr\Http\Message\ResponseInterface $response, \Symfony\Component\Serializer\SerializerInterface $serializer, string $contentType = null)
    {
        ...
        if (false === is_null($contentType) && (200 === $status && false !== mb_strpos($contentType, 'application/json'))) {
            return $serializer->deserialize($body, 'RevoTale\\CheckboxUA\\Model\\OperationBaseResponseSchema', 'json');
        }
        if (false === is_null($contentType) && (422 === $status && false !== mb_strpos($contentType, 'application/json'))) {
            throw new \RevoTale\CheckboxUA\Exception\ExportGoodsApiV1GoodsExportExportExtensionGetUnprocessableEntityException($serializer->deserialize($body, 'RevoTale\\CheckboxUA\\Model\\HTTPValidationError', 'json'), $response);
        }
