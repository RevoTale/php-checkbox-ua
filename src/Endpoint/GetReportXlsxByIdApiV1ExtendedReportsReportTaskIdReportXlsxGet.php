...
class GetReportXlsxByIdApiV1ExtendedReportsReportTaskIdReportXlsxGet extends \RevoTale\CheckboxUA\Runtime\Client\BaseEndpoint implements \RevoTale\CheckboxUA\Runtime\Client\Endpoint
{
    use \RevoTale\CheckboxUA\Runtime\Client\EndpointTrait;
    protected $report_task_id;
     ...
     *             }
     */
    public function __construct(string $reportTaskId, array $headerParameters = [])
    {
        $this->report_task_id = $reportTaskId;
    ...
    {
        return str_replace(['{report_task_id}'], [$this->report_task_id], '/api/v1/extended-reports/{report_task_id}/report.xlsx');
    }
    ...
    protected function transformResponseBody(\Psr\Http\Message\ResponseInterface $response, \Symfony\Component\Serializer\SerializerInterface $serializer, string $contentType = null)
    {
        ...
        if (false === is_null($contentType) && (200 === $status && false !== mb_strpos($contentType, 'application/json'))) {
            return $serializer->deserialize($body, 'RevoTale\\CheckboxUA\\Model\\PublicReportTaskSchema', 'json');
        }
        if (false === is_null($contentType) && (422 === $status && false !== mb_strpos($contentType, 'application/json'))) {
            throw new \RevoTale\CheckboxUA\Exception\GetReportXlsxByIdApiV1ExtendedReportsReportTaskIdReportXlsxGetUnprocessableEntityException($serializer->deserialize($body, 'RevoTale\\CheckboxUA\\Model\\HTTPValidationError', 'json'), $response);
        }
